...
public final class DownloadStorage: DownloadStorageManaging, @unchecked Sendable {
    ...
    public init() async throws {
        ...
        self.ioQueue = DispatchQueue(label: "com.SRNetworkManager.storage.io", qos: .utility)
        
            // Get documents directory
        let documentsURL = fileManager.urls(for: .documentDirectory, in: .userDomainMask)[0]
        self.baseDirectory = documentsURL.appendingPathComponent("Downloads", isDirectory: true)
    ...
    public func getDirectoryFor(task: DownloadTask) -> URL {
        baseDirectory.appendingPathComponent(task.id.uuidString, isDirectory: true)
    }
    ...
    public func saveFile(at sourceURL: URL, for task: DownloadTask) async throws {
        try await withCheckedThrowingContinuation { continuation in
            ioQueue.async {
                do {
                    let directory = self.getDirectoryFor(task: task)
                    let destinationURL = directory.appendingPathComponent(task.fileName)
    ...
    public func removeTask(_ taskId: UUID) async throws {
        try await withCheckedThrowingContinuation { continuation in
            ioQueue.async {
                do {
                    let directory = self.baseDirectory.appendingPathComponent(taskId.uuidString, isDirectory: true)
                    if self.fileManager.fileExists(atPath: directory.path) {
